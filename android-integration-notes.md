...
输入栏原有表情占位按钮已替换为响应式 `EmojiPicker`。表情数据按最近使用、笑脸与人物、手势与身体、动物与自然、食物与饮品、活动与庆祝、旅行与地点、物品、符号与爱心分类；搜索同时匹配中文、英文关键词和表情字符。最近使用最多保留 24 个并只写入设备本地 `localStorage`。面板使用键盘可访问标签、Esc/外部点击关闭和视口内最大高度；触摸设备打开时不主动聚焦搜索框，避免 Android/鸿蒙软键盘遮挡面板。

选中的 Unicode 表情以独立 `MessageKind.Emoji` 发送，但其明文处理与普通文字完全相同：客户端使用当前 `keyVersion` 的 AES-GCM 会话密钥加密，服务端只保存密文、随机数和类型。消息气泡在解密后使用大号无底色样式；会话预览固定为“[表情]”，本地/FCM 通知固定为“发来一个表情”，不把实际表情字符写入通知载荷。这样既能让客户端选择专用视觉样式，也不削弱现有端到端加密边界。
