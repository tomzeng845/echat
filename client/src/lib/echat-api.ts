...
export async function uploadMedia(file: Blob, fileName: string, purpose: "Chat" | "Moment", conversationId?: string) {
...
export function connectRealtime(handlers: RealtimeHandlers) {
  ...
  if (handlers.onCallSignal) connection.on("call.signal", handlers.onCallSignal);
