...
当前代码、Web/iOS 同步、plist/资源静态检查、APNs 单元测试、双平台设备 API、私有 GitHub 仓库、`ios-production` 环境、Distribution 证书、production profile 和签名 IPA 均已就绪。最终 IPA 为 `releases/EChat-0.9.0-TestFlight.ipa`，大小 6,957,995 bytes，SHA-256 为 `e99ffe0e7b4d658eb9cbc7ef83915cc161fa5dd1ac947b334573b7e2f3eb044d`。**当前下一步**是创建 App Store Connect 记录和上传 API Key，再上传构建；APNs Key、Apple 处理、测试员分配和真机验收也尚未完成。TestFlight 构建可测试 90 天，内部测试最多 100 人；首次外部测试还需要 Beta App Review。详见 [GitHub IPA 构建说明](github-ios-ipa.md)、[Apple 配置详细指南](apple-app-store-connect-setup-guide.md)、[iOS/TestFlight 发布说明](ios-testflight-notes.md)、[Apple 上传构建文档](https://developer.apple.com/help/app-store-connect/manage-builds/upload-builds/)和 [Apple TestFlight 文档](https://developer.apple.com/help/app-store-connect/test-a-beta-version/testflight-overview)。
