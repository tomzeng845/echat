...
iOS 客户端位于 `ios/`，Bundle ID 为 `com.tomzeng845.echat`，版本为 `0.9.0 (181)`，最低系统版本为 iOS 15。Android 继续使用已发布包名 `com.echat.app`，两端标识相互独立。工程已包含 1024×1024 App Store 图标、启动图、相机/麦克风/相册权限说明、`audio`/`remote-notification`/`voip` 后台模式、APNs entitlement 和 Apple 隐私清单。普通消息、好友申请与推送测试使用 APNs alert；真实音视频邀请才使用 PushKit VoIP push，并立即报告给 CallKit。用户在系统来电界面接听时会恢复 WebRTC 接听流程，拒接、远端接听或挂断会按 `callId` 清理其他 iOS 设备上的系统来电。

Linux 可以同步和静态验证 iOS 工程，但不能运行 Xcode Archive 或真机 CallKit 测试。Team `PPY8H6QWB5` 已注册 `com.tomzeng845.echat` 显式 App ID 并开启 Push Notifications；Apple Distribution 证书与 `EChat App Store 2026` production profile 已创建并验证。私有仓库 `tomzeng845/echat` 的 GitHub-hosted macOS 26 workflow `34028293524` 已成功生成并上传 App Store Connect 分发签名的 `0.9.0 (181)` IPA。App Store Connect 应用记录为“E聊即时通讯”（Apple ID `6809145695`）；Apple 已处理构建、出口合规已完成，“E聊内部测试”组已包含该构建并邀请账号持有人。旧 APNs Key `35STBCJUCJ` 已撤销，原 `5FLA6SLK3N` 确认为 Sandbox；新 Production Key `35Z9TC62QQ`（`EChat APNs Production 2026 v4`）已安全加载正式服务，重启后的 `/api/health` 与部署 Vitest 已确认 `iosEnabled=true`、provider 为 Apple APNs。当前剩余工作是 TestFlight 设备重新注册 `ios-voip` token，以及普通通知、PushKit/CallKit、摄像头、麦克风和双向媒体验收。
...
仓库现在还包含手工触发的 `.github/workflows/ios-ipa.yml`。它固定使用 GitHub-hosted `macos-26`、Apple Distribution `.p12` 和 App Store Connect production provisioning profile，验证 Team、Bundle ID、APNs entitlement、代码签名、版本和 Build 后，把 IPA、SHA-256 与构建元数据保存为 14 天的私有 workflow artifact。`upload_testflight` 默认关闭；只有人工明确开启且三项上传 API Key Secrets 均存在时，workflow 才会验证并上传构建。凭据必须放入受保护的 `ios-production` Environment Secrets。详见 [GitHub IPA 构建说明](github-ios-ipa.md)。
...
当前代码、Web/iOS 同步、plist/资源静态检查、APNs 单元测试、双平台设备 API、私有 GitHub 仓库、`ios-production` 环境、Distribution 证书、production profile、App Store Connect 记录、签名 IPA、TestFlight 上传、Apple 处理、出口合规、内部测试邀请和正式 APNs 健康启用均已完成。最终 IPA 为 `releases/EChat-0.9.0-TestFlight.ipa`，大小 6,957,997 bytes，SHA-256 为 `d2f53d9a4b72387b5c7500c3f7d157b6b1af39f3bbdeb902656e444d8c024542`。**当前下一步**是在账号持有人的 iPhone 安装 TestFlight、接受邀请完成真机通知、PushKit/CallKit、摄像头、麦克风和双向媒体验收。TestFlight 构建可测试 90 天，内部测试最多 100 人；首次外部测试还需要 Beta App Review。详见 [GitHub IPA 构建说明](github-ios-ipa.md)、[Apple 配置详细指南](apple-app-store-connect-setup-guide.md)、[iOS/TestFlight 发布说明](ios-testflight-notes.md)、[Apple 上传构建文档](https://developer.apple.com/help/app-store-connect/manage-builds/upload-builds/)和 [Apple TestFlight 文档](https://developer.apple.com/help/app-store-connect/test-a-beta-version/testflight-overview)。
