...
当前代码、Web/iOS 同步、plist/资源静态检查、APNs 单元测试、双平台设备 API、私有 GitHub 仓库、`ios-production` 环境、Distribution 证书、production profile、App Store Connect 记录、签名 IPA、TestFlight 上传、Apple 处理、出口合规和内部测试邀请均已完成。最终 IPA 为 `releases/EChat-0.9.0-TestFlight.ipa`，大小 6,957,997 bytes，SHA-256 为 `d2f53d9a4b72387b5c7500c3f7d157b6b1af39f3bbdeb902656e444d8c024542`。**当前下一步**是将正确 APNs p8 注入正式 API，并在账号持有人的 iPhone 安装 TestFlight、接受邀请完成真机验收；隔离环境已验证该 p8 可使 `iosEnabled=true`，但正式 API 仍为 `iosEnabled=false`。TestFlight 构建可测试 90 天，内部测试最多 100 人；首次外部测试还需要 Beta App Review。详见 [GitHub IPA 构建说明](github-ios-ipa.md)、[Apple 配置详细指南](apple-app-store-connect-setup-guide.md)、[iOS/TestFlight 发布说明](ios-testflight-notes.md)、[Apple 上传构建文档](https://developer.apple.com/help/app-store-connect/manage-builds/upload-builds/)和 [Apple TestFlight 文档](https://developer.apple.com/help/app-store-connect/test-a-beta-version/testflight-overview)。
